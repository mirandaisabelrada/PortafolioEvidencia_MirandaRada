...
```{r}
df_sinNA<-read.csv("C:/Users/OFICINA/Documents/R studio/MULTIVARIADOS/RETO/Bases de datos Estacion/San Pedro.csv")
...
library(lme4)
library(Matrix)
mod_CO <- lmer(CO ~ NOX+NO2+season+ (1 | pandemia)+ (1| WSR) +(1 | WDR), data = df_sinNA)
summary(mod_CO)
```
...
ggplot(df_sinNA, aes(x = resid(mod_CO))) +
...
library(lme4)
library(Matrix)
mod_NO <- lmer(NO ~  NO2+NOX+season + (1 | pandemia )+ (1 | WSR) +(1 | WDR), data = df_sinNA)
summary(mod_NO)
```
```{r}
...
library(lme4)
library(Matrix)
mod_NO2 <- lmer(NO2 ~ NO+NOX+PM10+PM2.5+SO2+CO+season + (1 | pandemia)+ (1 | WSR) +(1 | WDR), data = df_sinNA)
summary(mod_NO2)
```
...
library(lme4)
library(Matrix)
mod_NOX <- lmer(NOX ~ NO2+NO+PM10+PM2.5+CO+TOUT+SO2+season + (1 | pandemia) + (1 | WSR) +(1 | WDR), data = df_sinNA)
summary(mod_NOX)
```
...
library(lme4)
library(Matrix)
mod_O3 <- lmer(O3 ~ RH+SR+TOUT+season + (1 | pandemia) + (1 | WSR) +(1 | WDR), data = df_sinNA)
summary(mod_O3)
```
```{r}
...
library(lme4)
library(Matrix)
mod_PM10 <- lmer(PM10 ~ NO2+NOX+PM2.5+season + (1 | pandemia)+ (1 | WSR) +(1 | WDR), data = df_sinNA)
summary(mod_PM10)
```
...
library(lme4)
library(Matrix)
mod_PM2.5 <- lmer(PM2.5 ~ NO2+NOX+PM10+SR+season + (1 | pandemia)+ (1 | WSR) +(1 | WDR), data = df_sinNA)
summary(mod_PM2.5)
```
```{r}
...
library(lme4)ikkk
library(Matrix)
mod_SO2 <- lmer(SO2 ~ NO2+NOX+season+ (1 | pandemia)+ (1 | WSR) +(1 | WDR), data = df_sinNA)
summary(mod_SO2)
```

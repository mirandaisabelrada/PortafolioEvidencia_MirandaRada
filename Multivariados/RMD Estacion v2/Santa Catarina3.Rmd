...
```{r}
df_sinNA<-read.csv("C:/Users/OFICINA/Documents/R studio/MULTIVARIADOS/RETO/Bases de datos Estacion/Santa Catarina.csv")
...
library(lme4)
library(Matrix)
mod_CO <- lmer(CO ~ + NO + NO2+NOX+PM10+PM2.5+season + (1 | pandemia)+ (1| WSR) +(1 | WDR), data = df_sinNA)
summary(mod_CO)
```
...
library(lme4)
library(Matrix)
mod_NO <- lmer(NO ~ + CO + NO2+NOX+PM10+PM2.5+season + (1 | pandemia )+ (1 | WSR) +(1 | WDR), data = df_sinNA)
summary(mod_NO)
```

...
```{r}
#Creación del df con los datos. 
df_sinNA<-read.csv("/Users/mirisrc/Downloads/Cadereyta.csv")
df_sinNA<-na.omit(df_sinNA)
head(df_sinNA)
```
# División de datos en entrenamiento y testeo
```{r}
#Creación del gráfico de correlaciones. 
library(corrplot)
...
```{r}
#Función para ver cuales correlaciones tienen un valor mayor a 0.3. 
library(dplyr)
...
library(lme4)
library(Matrix)
mod_CO <- lmer(CO ~ NOX+NO+NO2+ PM10+season+ (1 | pandemia)+ (1| WSR) +(1 | WDR), data = df_sinNA)
summary(mod_CO)
```
...
library(lme4)
library(Matrix)
mod_NO <- lmer(NO ~  NO2+NOX+O3+PM10+PM2.5+CO+season + (1 | pandemia )+ (1 | WSR) +(1 | WDR), data = df_sinNA)
summary(mod_NO)
```
```{r}
...
library(lme4)
library(Matrix)
mod_NO2 <- lmer(NO2 ~ NO+NOX+PM10+PM2.5+TOUT+O3+CO+season + (1 | pandemia)+ (1 | WSR) +(1 | WDR), data = df_sinNA)
summary(mod_NO2)
```
...
library(lme4)
library(Matrix)
mod_NOX <- lmer(NOX ~ NO2+NO+O3+PM10+PM2.5+CO+TOUT+season + (1 | pandemia) + (1 | WSR) +(1 | WDR), data = df_sinNA)
summary(mod_NOX)
```
...
library(lme4)
library(Matrix)
mod_O3 <- lmer(O3 ~ NOX+ NO+NO2+SO2+RH+SR+TOUT+season + (1 | pandemia) + (1 | WSR) +(1 | WDR), data = df_sinNA)
summary(mod_O3)
```
...
library(lme4)
library(Matrix)
mod_PM10 <- lmer(PM10 ~ +  CO+NO2+NOX+NO+PM2.5+season + (1 | pandemia)+ (1 | WSR) +(1 | WDR), data = df_sinNA)
summary(mod_PM10)
```
...
library(lme4)
library(Matrix)
mod_PM2.5 <- lmer(PM2.5 ~ NO2+NOX+PM10+NO+season + (1 | pandemia)+ (1 | WSR) +(1 | WDR), data = df_sinNA)
summary(mod_PM2.5)
```
```{r}
...
library(lme4)
library(Matrix)
mod_SO2 <- lmer(SO2 ~ O3+SR+season+ (1 | pandemia)+ (1 | WSR) +(1 | WDR), data = df_sinNA)
summary(mod_SO2)
```
